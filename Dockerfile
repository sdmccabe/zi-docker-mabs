...
RUN apt-get update
RUN apt-get install -yqq openjdk-7-jdk golang erlang erlang-doc ghc ghc-prof ghc-doc scala sbt leiningen time git cabal-install curl build-essential checkinstall libgflags-dev python-dev python-setuptools python-pip
RUN apt-get clean
RUN rm -rf /var/lib/apt/lists/* /tmp/* /var/tmp/*
