...
func main() {
	...
	flag.IntVar(&numThreads, "t", runtime.NumCPU()*2, "number of goroutine to use")
	flag.BoolVar(&verbose, "v", false, "verbose (track goroutines)")
	flag.BoolVar(&profiling, "profile", false, "enable CPU profiling")
	flag.IntVar(&numBuyers, "a", 10000, "number of agents")
	flag.IntVar(&maxNumberOfTrades, "n", 1000000, "maximum number of trades")
	flag.Parse()
	numSellers = numBuyers // lazy approach to setting agents
	...
	fmt.Printf("numBuyers: %d\n", numBuyers)
	fmt.Printf("numSellers: %d\n", numSellers)
	fmt.Printf("maxNumberOfTrades: %d\n", maxNumberOfTrades)
