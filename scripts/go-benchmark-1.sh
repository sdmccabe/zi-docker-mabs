...
cd /scripts/
echo "threads,real,user,sys,memory" > results/experiment-1/go-benchmark.csv
...
for ((i=1; i<10; i++))
do 
{ /usr/bin/time -f "${threads},%e,%U,%S,%M" /models/go/zi-traders -n=1000000 -a=10000 -t=$threads ; } 2>> results/experiment-1/go-benchmark.csv
done
done