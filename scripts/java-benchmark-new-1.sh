...
cd /scripts/
echo "threads,real,user,sys,memory" > results/experiment-1/java-new-benchmark.csv
...
for ((i=1; i<10; i++))
do 
{ /usr/bin/time -f "${threads},%e,%U,%S,%M" java -jar /models/java/build/ZItraders.jar 10000 1000000 $threads new; } 2>> results/experiment-1/java-new-benchmark.csv
done
done