...
cd /scripts/
echo "threads,real,user,sys,memory" > results/experiment-2/go-benchmark.csv
...
for ((i=0; i<10; i++))
do 
{ /usr/bin/time -f "${threads},%e,%U,%S,%M" /models/go/zi-traders -n=10000000 -a=100000 -t=$threads ; } 2>> results/experiment-2/go-benchmark.csv
done
done