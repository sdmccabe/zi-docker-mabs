...
cd /scripts/
echo "threads,real,user,sys,memory" > results/experiment-2/java-new-benchmark.csv
...
for ((i=0; i<10; i++))
do 
{ /usr/bin/time -f "${threads},%e,%U,%S,%M" java -jar /models/java/build/ZItraders.jar 100000 10000000 $threads new; } 2>> results/experiment-2/java-new-benchmark.csv
done
done