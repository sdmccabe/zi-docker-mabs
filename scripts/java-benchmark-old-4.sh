...
cd /scripts/
echo "threads,real,user,sys,memory" > results/experiment-4/java-old-benchmark.csv
...
for ((i=0; i<10; i++))
do 
{ /usr/bin/time -f "${threads},%e,%U,%S,%M" java -jar /models/java/build/ZItraders.jar 10000000 1000000000 $threads old; } 2>> results/experiment-4/java-old-benchmark.csv
done
done